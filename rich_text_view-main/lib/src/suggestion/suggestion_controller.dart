  ...
  void onChanged(String? value) async {
    ...
    if (_last.isNotEmpty && (isHash || isMention)) {
      if (_last.length == 1) {
        ...
        List<Mention> temp = [];
        if (onSearchMention != null && _last.length > 1) {
          temp = await onSearchMention!(_last.split(mentionSymbol)[1]);

          temp = temp.where((e) {
            print(e.title);
            return e.title.toLowerCase().contains(rate);
          }).toList();
        } else {
          temp = intialMentions == null
              ? []
              : intialMentions!
                  .where((e) => e.title.toLowerCase().contains(rate))
                  .toList();
        }
        clear(people: temp ?? []);
      } else if (isHash) {
